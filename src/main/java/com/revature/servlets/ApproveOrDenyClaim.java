...
/**
 * Servlet implementation class ApproveOrDenyClaim
 */
public class ApproveOrDenyClaim extends HttpServlet {
	...
	protected void directControlRouter(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException, UnauthorizedException, UnauthenticatedException {
		...
		switch (URI) {
			...
			switch (req.getMethod()) {
				...
				case "POST":{
					userController.aprroveReimbursement(req, res);
					break;
				}
				case "PUT":{
					userController.denyReimbursement(req, res);
					break;
				}
				case "DELETE":{
					res.setStatus(400);
