...
/**
 * Servlet implementation class AllReimbursementClaims
 */
public class AllReimbursementClaims extends HttpServlet {
		...
		protected void directControlRouter(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException, UnauthorizedException, UnauthenticatedException {
			...
			switch (URI) {
				...
				switch (req.getMethod()) {
					...
					case "POST":{
						userController.viewAllReimbursementsClaims(req, res);
						break;
					}
					case "PUT":{
						res.setStatus(400);
